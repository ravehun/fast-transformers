...
void run_n_with_timer(int n, void *fn ()) {
    long now = 0;
...
//            arb_hypgeom_pfq(r, a, 1, b, 1, z, 0, prec);
            arb_hypgeom_pfq(r, av, 3, bv, 4, z, 0, prec);
        }
        flint_printf("arb_hypgeom_pfq: %d ms\n", int(timer()-now));
        double result = arf_get_d(arb_midref(r), ARF_RND_NEAR);
        flint_printf("result: %f\n", result);
...
//        flint_printf("arb_hypgeom_bessel_k: %ld ms\n", (timer()-now) / 1000);
           now = timer();
          for(int i = 0; i < 1500; i++){
            arb_hypgeom_bessel_k(r, a, z, prec);
          }
          printf("bessel_k %ld ms\n", (timer()-now));
          arb_set_d(a, 0.01);
          arb_set_d(v, 1.1);
          arb_hypgeom_bessel_k(r, v,a, prec);
          flint_printf("bessel_k(0.01) is %s\n", arb_get_str(r,prec,ARB_STR_MORE));

          arb_set_d(a, 0.01);
          arb_digamma(r,a, prec);
          flint_printf("digamma(0.01) is %s", arb_get_str(r,prec,ARB_STR_MORE));
//        printf("%f\n", average_apply(lambda(float,(float x),{ return 2*x; })));
//        flint_printf("During: %ld\n", (timer()-now) / 1000);
